...
log4js.configure(
    {
        appenders: {
            ...
            log_tools: {
                type : 'file',
                filename: './logs/log_tools.log',//文件目录，当目录文件或文件夹不存在时，会自动创建
            ...
            log_router: {
                type : 'file',
                filename: './logs/log_router.log',//文件目录，当目录文件或文件夹不存在时，会自动创建
                maxLogSize : 104800,//文件最大存储空间，当文件内容超过文件存储空间会自动生成一个文件test.log.1的序列自增长的文件
                backups : 3,//当文件内容超过文件存储空间时，备份文件的数量
                encoding : 'utf-8',//default "utf-8"，文件的编码
                category : 'log_router',
            ...
            log_dao: {
                type : 'file',
                filename: './logs/log_dao.log',//文件目录，当目录文件或文件夹不存在时，会自动创建
                maxLogSize : 104800,//文件最大存储空间，当文件内容超过文件存储空间会自动生成一个文件test.log.1的序列自增长的文件
                backups : 3,//当文件内容超过文件存储空间时，备份文件的数量
                encoding : 'utf-8',//default "utf-8"，文件的编码
                category : 'log_dao',
        ...
        categories: {
            ...
            log_tools : { appenders :['log_tools'],  level: 'info' },
            log_router : { appenders :['log_router'], level: 'info' },
            log_dao : { appenders :['log_dao'], level: 'info'  }
        }
    }
);
...
//             //     "pattern": "-yyyy-MM-dd-hh.log"  
//             // } 
//             //介绍的网址 ： http://blog.csdn.net/lxhjh/article/details/50747642